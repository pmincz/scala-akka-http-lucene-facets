...
import com.test.domain.{Show, ShowDetail, ShowResult}
...
object ShowClient extends ActorSystemProvider with JsonSupport {
  ...
  def getShowDetail(id: Long): Future[ShowDetail] = {
    val url = s"${domain}/tv/${id}?api_key=${key}"
    Http().singleRequest(HttpRequest(uri = url)).flatMap { r =>
      Unmarshal(r.entity).to[ShowDetail]
    }
  }
